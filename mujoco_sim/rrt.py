...
SAVE_DIR = os.path.join(CURR_DIR, "optimal_path_rrt")
...
def main():
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while rrt.samples_count < MAX_SAMPLES:
            ...
            print("================ RRT* LIVE MONITORING ================")
            print(f" Samples: {rrt.samples_count}/{MAX_SAMPLES} | Nodes: {len(rrt.nodes)}")
            print(f" Status:  {'[ GOAL REACHED ]' if rrt.goal_found else '[ EXPLORING ]'}")
            print("-" * 54)
            if rrt.discovery_log:
                print(" Recent Optimization Discoveries:")
                for log in rrt.discovery_log[-3:]:
                    print(f"  [+] {log}")
            else:
                print(" Waiting for initial path discovery...")
            print("======================================================")
