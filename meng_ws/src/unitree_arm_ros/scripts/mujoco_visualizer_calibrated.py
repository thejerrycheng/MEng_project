...
# These names must match the order of data in self.qpos
MJ_JOINT_NAMES = [
    "joint_1","joint_2","joint_3",
...
class MujocoStateVisualizer:
    def __init__(self):
        ...
        # NEW: Publisher for the calibrated states
        self.pub_calib = rospy.Publisher("/calibrated_joint_states", JointState, queue_size=10)
        
        rospy.Subscriber("/joint_states", JointState, self.joint_cb, queue_size=1)

        rospy.loginfo("==============================================")
        rospy.loginfo(" MuJoCo Visualizer & Calibrated Publisher Running ")
        rospy.loginfo("==============================================")
    ...
    def joint_cb(self, msg):
        ...
        # 1. Calculate Calibrated Positions
        # Base joints
        for i in range(4):
        ...
        # 2. Publish Calibrated Topic
        calib_msg = JointState()
        calib_msg.header.stamp = rospy.Time.now()
        # Use MJ_JOINT_NAMES because self.qpos now contains pitch/roll, not joint_5/6
        calib_msg.name = MJ_JOINT_NAMES 
        calib_msg.position = self.qpos.tolist()
