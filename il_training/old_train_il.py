...
class EpisodeWindowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        q_last = ep["joints"][start + self.seq_len - 1]     # last observed joint
        fut = ep["joints"][start + self.seq_len : start + self.seq_len + self.future_steps]
        fut_delta = fut - q_last[None, :]                   # Δq target
        future_joints = torch.tensor(np.asarray(fut_delta), dtype=torch.float32)
...
def run_epoch(model, loader, device, optimizer=None,
    ...
    for batch in loader:
        ...
        q_pred_last = q_last + pred[:, -1, :] # convert Δq → absolute
