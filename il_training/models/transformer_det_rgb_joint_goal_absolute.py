import torch
import torch.nn as nn
from torchvision import models

# Constants
NUM_JOINTS = 6

# --- 1. Helper: Spatial Softmax ---
class SpatialSoftmax(nn.Module):
    def __init__(self, height: int, width: int, temperature: float = 1.0):
        super().__init__()
        self.height = height
        self.width = width
        self.temperature = temperature
        pos_x, pos_y = torch.meshgrid(
            torch.linspace(-1, 1, height),
            torch.linspace(-1, 1, width),
            indexing='ij'
        )
        self.register_buffer("pos_x", pos_x.reshape(-1))
        self.register_buffer("pos_y", pos_y.reshape(-1))

    def forward(self, feature_map):
        B, C, H, W = feature_map.shape
        flat = feature_map.view(B, C, -1)
        attention = torch.nn.functional.softmax(flat / self.temperature, dim=-1)
        expected_x = torch.sum(self.pos_x * attention, dim=2)
        expected_y = torch.sum(self.pos_y * attention, dim=2)
        return torch.cat([expected_x, expected_y], dim=1)

# --- 2. Main Model: Simplified Absolute Transformer ---
class Transformer_Absolute(nn.Module):
    def __init__(
        self,
        seq_len: int,          
        future_steps: int,     
        d_model: int = 256,
        nhead: int = 8,
        enc_layers: int = 4,
        dec_layers: int = 4,
        ff_dim: int = 1024,
        dropout: float = 0.1,
    ):
        super().__init__()
        self.d_model = d_model

        # --- Vision Backbone (ResNet18) ---
        resnet = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
        self.backbone = nn.Sequential(*list(resnet.children())[:-2]) 
        
        # Freeze Backbone (Optional: Comment out to fine-tune)
        for param in self.backbone.parameters():
            param.requires_grad = False
            
        self.spatial_softmax = SpatialSoftmax(height=7, width=7)
        self.vision_feature_dim = 512 * 2

        # --- Projections ---
        # 1. Project Visual Features to Transformer Dim
        self.rgb_proj = nn.Sequential(
            nn.Linear(self.vision_feature_dim, d_model),
            nn.LayerNorm(d_model),
            nn.Dropout(dropout),
        )
        
        # 2. Project Joint History (if available) to Transformer Dim
        self.joint_proj = nn.Sequential(
            nn.Linear(NUM_JOINTS, d_model),
            nn.LayerNorm(d_model),
        )

        # 3. Fusion (Vision + Joint History)
        self.fusion_proj = nn.Sequential(
            nn.Linear(d_model * 2, d_model),
            nn.LayerNorm(d_model),
            nn.GELU()
        )

        # --- Transformer ---
        # +1 for Goal Token
        self.enc_pos = nn.Parameter(torch.randn(seq_len + 1, d_model) * 0.02)
        
        # Queries for future steps
        self.action_queries = nn.Parameter(torch.randn(future_steps, d_model) * 0.02)
        self.dec_pos = nn.Parameter(torch.randn(future_steps, d_model) * 0.02)

        self.transformer = nn.Transformer(
            d_model=d_model,
            nhead=nhead,
            num_encoder_layers=enc_layers,
            num_decoder_layers=dec_layers,
            dim_feedforward=ff_dim,
            dropout=dropout,
            batch_first=True,
            norm_first=True,
        )

        # Output Head: Predicts ABSOLUTE joint positions
        self.head = nn.Sequential(
            nn.LayerNorm(d_model),
            nn.Linear(d_model, NUM_JOINTS),
            # Optional: Add Tanh if you normalized your data to [-1, 1]
            # nn.Tanh() 
        )

    def forward(self, rgb_seq, joint_seq, goal_image):
        """
        Args:
            rgb_seq: (B, S, 3, H, W) - History of images
            joint_seq: (B, S, 6) - History of current joint positions
            goal_image: (B, 3, H, W) - The target image
        Returns:
            pred_absolute_joints: (B, Future_Steps, 6)
        """
        B, S, C, H, W = rgb_seq.shape
        
        # --- 1. Vision Encoding ---
        # Flatten Sequence: (B*S, C, H, W)
        x_seq = rgb_seq.view(B * S, C, H, W)
        
        # Pass through backbone
        with torch.no_grad(): # Assuming frozen backbone
            feat_map = self.backbone(x_seq)
        
        # Spatial Softmax & Project
        feat_coords_seq = self.spatial_softmax(feat_map).view(B, S, -1)
        rgb_tok = self.rgb_proj(feat_coords_seq) # (B, S, D)
        
        # --- 2. Goal Encoding ---
        with torch.no_grad():
            goal_feat = self.backbone(goal_image)
        feat_coords_goal = self.spatial_softmax(goal_feat)
        goal_tok = self.rgb_proj(feat_coords_goal).unsqueeze(1) # (B, 1, D)

        # --- 3. Joint History Encoding ---
        joint_tok = self.joint_proj(joint_seq) # (B, S, D)
        
        # --- 4. Fusion ---
        # Combine visual history with proprioception history
        enc_tok = self.fusion_proj(torch.cat([rgb_tok, joint_tok], dim=-1))
        
        # Append Goal Token to the end
        # Sequence: [Obs_1, Obs_2, ..., Obs_N, Goal]
        enc_tok = torch.cat([enc_tok, goal_tok], dim=1)

        # Add Positional Embeddings
        enc_tok = enc_tok + self.enc_pos.unsqueeze(0)

        # --- 5. Transformer Decoder ---
        # Queries represent the future time steps we want to predict
        q = self.action_queries.unsqueeze(0).expand(B, -1, -1) + self.dec_pos.unsqueeze(0)
        
        dec_out = self.transformer(enc_tok, q)
        
        # Prediction
        pred_absolute_joints = self.head(dec_out) 
        
        return pred_absolute_joints