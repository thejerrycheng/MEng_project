...
# --- 1. Helper: Spatial Softmax (Same as before) ---
...
# --- 2. New Component: CVAE Encoder ---
class CVAEEncoder(nn.Module):
    """
    Takes the future actions (Ground Truth) and compresses them into a latent 'z'.
    Only used during TRAINING.
    """
    def __init__(self, action_dim, hidden_dim, latent_dim):
        super().__init__()
        # Input: Action dimension * Sequence Length (flattened) + Conditioning (Current Joint)
        # We simplify here to just encoding the Action Sequence for the style
    ...
    def forward(self, actions, current_joints):
        """
        actions: (B, Future_Steps, Num_Joints)
        current_joints: (B, Num_Joints)
        """
        B = actions.shape[0]
        # Flatten actions: (B, Future_Steps * 6)
        flat_actions = actions.view(B, -1)
        
        # Condition on current state (simple concatenation)
...
# --- 3. Main Model: ACT with CVAE ---
class ACT_CVAE_Optimized(nn.Module):
    def __init__(
        self,
        seq_len: int,          
        future_steps: int,     
        ...
        ff_dim: int = 1024,
        dropout: float = 0.1,
        latent_dim: int = 32, # Size of the CVAE z vector
    ):
        super().__init__()
        ...
        # Input size = (Future_Steps * 6) + (Current Joint 6)
        ...
        # 2. The Projection (Latent -> Transformer Size)
        self.latent_proj = nn.Linear(latent_dim, d_model)
        
        # --- Transformer ---
        # +2: +1 for Goal Token, +1 for Latent Token
    ...
    def forward(self, rgb_seq, joint_seq, goal_image, target_actions=None):
        ...
        Args:
            ...
            joint_seq: (B, S, 6)
            goal_image: (B, 3, H, W)
            target_actions: (B, Future_Steps, 6) -> GROUND TRUTH. 
                            Required if training=True. None if inference.
        """
        ...
        if is_training:
            # A. Training: Encode the Future Actions + Current Joint to get z
            # current_joint is the last one in the sequence
            ...
            # B. Inference: Set z to zero (mean of prior)
            z = torch.zeros((B, self.latent_dim), device=rgb_seq.device)
            
        # Project z to be a token (B, 1, D)
        latent_tok = self.latent_proj(z).unsqueeze(1)

        # --- 2. Vision & Proprioception Encoding ---
        # (Same as previous model)
        x_seq = rgb_seq.view(B * S, C, H, W)
        feat_coords_seq = self.spatial_softmax(self.backbone(x_seq)).view(B, S, -1)
        rgb_tok = self.rgb_proj(feat_coords_seq)
        
        feat_coords_goal = self.spatial_softmax(self.backbone(goal_image))
        goal_tok = self.rgb_proj(feat_coords_goal).unsqueeze(1)
        
        joint_tok = self.joint_proj(joint_seq)
        
        # Concatenate Vision + Joint -> Fusion
        enc_tok = self.fusion_proj(torch.cat([rgb_tok, joint_tok], dim=-1))
        
        # --- 3. Assemble Transformer Input ---
        # Sequence: [Latent_Token, Observation_Tokens..., Goal_Token]
        # Adding Latent Token at the START acts as a "Style Conditioner"
        ...
        # Return prediction AND stats for loss calculation
