...
# --- Main Model: Partially Frozen Backbone ---
class ACT_CVAE_Partial(nn.Module):
    def __init__(
        self,
        seq_len: int,          
        future_steps: int,     
        ...
        # --- Vision Backbone (Partially Frozen ResNet18) ---
        resnet = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
        self.backbone = nn.Sequential(*list(resnet.children())[:-2]) 
        
        # PARTIAL FREEZE STRATEGY:
        # ResNet18 Sequential Layout:
        # 0: Conv1, 1: BN, 2: ReLU, 3: MaxPool
        # 4: Layer1 (BasicBlock) -> Freeze
        # 5: Layer2 (BasicBlock) -> Freeze
        # 6: Layer3 (BasicBlock) -> TRAINABLE
        # 7: Layer4 (BasicBlock) -> TRAINABLE
        
        # 1. Freeze Layers 0 to 5 (Early features: edges, textures)
        for layer in self.backbone[:6]:
            for param in layer.parameters():
                param.requires_grad = False
        
        # 2. Layers 6 and 7 remain TRAINABLE (High level: object parts)
        # No loop needed for them, requires_grad is True by default.
    ...
    def forward(self, rgb_seq, joint_seq, goal_image, target_actions=None):
        ...
        # Note: No 'with torch.no_grad()' here because later layers are trainable!
        # Gradients will flow through Layer 4 -> Layer 3 -> stop at Layer 2.
